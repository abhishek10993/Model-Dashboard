...
public class ModelFIleRepositoryTest {
  ...
  private Project testProject = Project.builder().name("test project").build();
  private ModelMetadata testModelMetadata =
      DefaultDataBuilder.getNewMetadataBuilder().author(null).build();
  private Model testModel =
      Model.builder().modelMetadata(testModelMetadata).project(testProject).build();

  private DBFile testDbFile =
      DBFile.builder().fileName("testFile").fileType("testFileType").build();
  ...
  @Test
  public void shouldFindAllByProjectId() {
    List<ModelFile> modelFiles = modelFileRepository.findAllByProjectId(projectId);
    assertEquals(Collections.singletonList(testModelFile), modelFiles);
  }
}
