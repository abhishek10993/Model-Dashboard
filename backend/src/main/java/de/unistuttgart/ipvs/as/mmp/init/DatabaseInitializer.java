...
public class DatabaseInitializer implements ApplicationListener<ApplicationStartedEvent> {
    ...
    private static final List<String> modelNamePrefixes = Arrays.asList("New", "Old", "Best", "Even better",
    ...
    private static final List<String> algorithmNames = Arrays.asList("J48", "Simple Logistic", "rpart", "randomForest",
            "RProp", "Naive Bayes", "Multilayer Perceptron", "Linear Regression", "Hoeffding Tree");

    private static final String[] eamBusinessProcesses = {"Development", "Purchase", "Marketing", "Acquire orders",
            "Production", "Sale", "Shipping", "After Sales"};
    ...
    private List<Project> createProjects(List<User> users) {
        ...
        for (int i = 0; i < projectUseCases.length; i++) {
            LocalDate startDate = LocalDate.now().minusYears(9);
    ...
    private List<Model> createForEachModelStatusModel(List<Project> projects, List<User> users) {
        ...
        projects.forEach(project -> {
            ...
            for (int modelGroupIndex = 0; modelGroupIndex < amountModelGroups; modelGroupIndex++) {
                ...
                LocalDate dateOfCreation = LocalDate.now().minusYears(9);
    ...
    private LocalDate getRandomDate(LocalDate dateMin) {
        int year = 365;
        return dateMin.plusDays((long) (Math.random()*((year)+1)));
    }
}
